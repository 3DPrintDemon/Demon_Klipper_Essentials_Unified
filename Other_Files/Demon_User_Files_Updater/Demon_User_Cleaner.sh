...
# demon_custom_expansion_v*.cfg
 ...
 if [ ! -d "$DIR" ]; then
     ...
     if [ "$(ls -A $DIR)" ]; then
         ...
         cp /home/$U/printer_data/config/Demon_User_Files/demon_user_settings_cleaner_variables_v*.cfg /home/$U/printer_data/config/Demon_User_Files/Previous_Versions --backup=numbered
         rm /home/$U/printer_data/config/Demon_User_Files/demon_user_settings_cleaner_variables_v*.cfg
